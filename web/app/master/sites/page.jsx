...
export default function SitesPage(){
  ...
  async function load(p = page, ps = pageSize, search = q){
    ...
    try{
      ...
      const data = await apiClient('/master/sites?' + params.toString());
  ...
  async function loadHubs(){ try{ const d = await apiClient('/master/hubs'); setHubs(Array.isArray(d)?d:[]); }catch(e){console.error(e);} }
  ...
  async function remove(id){ if(!confirm('Hapus site?')) return; try{ await apiClient(`/master/sites/${id}`, { method:'DELETE' }); await load(); }catch(err){ alert(err?.body?.message||err?.message||'Delete failed'); } }

  return (
    <Box sx={{ p:2 }}>
      <Box sx={{ display:'flex', justifyContent:'space-between', alignItems:'center', mb:2 }}>
        <Box>
          <Box component="h2" sx={{ m:0 }}>Sites</Box>
          <Box sx={{ color:'text.secondary', fontSize:13 }}>Manage sites and assign to hubs</Box>
        </Box>
        <Box sx={{ display:'flex', alignItems:'center', gap:1 }}>
          ...
          <Button variant="contained" sx={{ ml:1 }} startIcon={<AddIcon/>} onClick={openCreate}>Create Site</Button>
        </Box>
      </Box>
