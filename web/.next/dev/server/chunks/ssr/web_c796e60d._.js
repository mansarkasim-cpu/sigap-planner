...
var __TURBOPACK__imported__module__$5b$project$5d2f$web$2f$node_modules$2f$frappe$2d$gantt$2f$dist$2f$frappe$2d$gantt$2e$es$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/web/node_modules/frappe-gantt/dist/frappe-gantt.es.js [app-ssr] (ecmascript)");
...
function FrappeGanttChart({ pageSize = 2000 }) {
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$web$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useEffect"])(()=>{
        ...
        // create new gantt
        try {
            const gantt = new __TURBOPACK__imported__module__$5b$project$5d2f$web$2f$node_modules$2f$frappe$2d$gantt$2f$dist$2f$frappe$2d$gantt$2e$es$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["default"](containerRef.current, tasks, {
                view_mode: 'Day',
...
"[project]/web/node_modules/frappe-gantt/dist/frappe-gantt.es.js [app-ssr] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
class F {
    ...
    refresh() {
        this.bar_group.innerHTML = "", this.handle_group.innerHTML = "", this.task.custom_class ? this.group.classList.add(this.task.custom_class) : this.group.classList = [
            "bar-wrapper"
        ], this.prepare_values(), this.draw(), this.bind();
    }
    set_defaults(t, e) {
    ...
    draw_resize_handles() {
        ...
        if (this.handles = [], this.gantt.options.readonly_dates || (this.handles.push(f("rect", {
            x: t.getEndX() - e / 2,
    ...
    setup_click_event() {
        ...
        p.on(this.group, "mouseenter", (s)=>{
            ...
            this.action_completed || (this.group.classList.remove("active"), this.gantt.popup && this.gantt.popup.parent.classList.remove("hide"), this.gantt.trigger_event("double_click", [
                this.task
            ]));
        });
        let i = !1;
        p.on(this.group, "touchstart", (s)=>{
            ...
            s.preventDefault(), !this.action_completed && (this.group.classList.remove("active"), this.gantt.popup && this.gantt.popup.parent.classList.remove("hide"), this.gantt.trigger_event("double_click", [
                this.task
            ]));
        });
    }
    ...
    compute_start_end_date() {
        ...
        const s = t.getWidth() / this.gantt.config.column_width, r = d.add(i, s * this.gantt.config.step, this.gantt.config.unit);
        return {
            new_start_date: i,
            new_end_date: r
        };
...
const b = [
    ...
    popup: (n)=>{
        ...
        const t = d.format(n.task._start, "MMM D", n.chart.options.language), e = d.format(d.add(n.task._end, -1, "second"), "MMM D", n.chart.options.language);
        n.set_details(`${t} - ${e} (${n.task.actual_duration} days${n.task.ignored_duration ? " + " + n.task.ignored_duration + " excluded" : ""})<br/>Progress: ${Math.floor(n.task.progress * 100) / 100}%`);
    },
    popup_on: "click",
    readonly_progress: !1,
    readonly_dates: !1,
...
class B {
    ...
    setup_options(t) {
        ...
        if (this.config = {
            ...
            typeof this.options.ignore == "string" && (this.options.ignore = [
                this.options.ignore
            ]);
    ...
    change_view_mode(t = this.options.view_mode, e = !1) {
        ...
        let i, s;
        e && (i = this.$container.scrollLeft, s = this.options.scroll_to, this.options.scroll_to = null), this.options.view_mode = t.name, this.config.view_mode = t, this.update_view_scale(t), this.setup_dates(e), this.render(), e && (this.$container.scrollLeft = i, this.options.scroll_to = s), this.trigger_event("view_change", [
            t
        ]);
    }
    update_view_scale(t) {
    ...
    setup_date_values() {
        ...
        for(this.dates = [
            t
        ]; t < this.gantt_end;)t = d.add(t, this.config.step, this.config.unit), this.dates.push(t);
    }
    bind_events() {
    ...
    setup_layers() {
        ...
        for (let e of t)this.layers[e] = f("g", {
            class: e,
            append_to: this.$svg
        });
    ...
    make_grid_rows() {
        ...
        }), e = this.dates.length * this.config.column_width, i = this.options.bar_height + this.options.padding;
    ...
    make_side_header() {
        if (this.$side_header = this.create_el({
            classes: "side-header"
        }), this.$upper_header.prepend(this.$side_header), this.options.view_mode_select) {
    ...
    highlight_holidays() {
        ...
        if (this.options.holidays) for(let e in this.options.holidays){
            ...
            for(let r = new Date(this.gantt_start); r <= this.gantt_end; r.setDate(r.getDate() + 1))if (!(this.config.ignored_dates.find((a)=>a.getTime() == r.getTime()) || this.config.ignored_function && this.config.ignored_function(r)) && (i(r) || s && s(r))) {
                const a = d.diff(r, this.gantt_start, this.config.unit) / this.config.step * this.config.column_width, o = this.grid_height - this.config.header_height, h = d.format(r, "YYYY-MM-DD", this.options.language).replace(" ", "_");
                if (t[r]) {
   ...
   */ highlight_current() {
    ...
    make_arrows() {
        ...
        for (let t of this.tasks){
            ...
            e = t.dependencies.map((i)=>{
                ...
                if (!s) return;
                const r = new q(this, this.bars[s._index], // from_task
                this.bars[t._index]);
    ...
    get_closest_date() {
        ...
        return [
            /* @__PURE__ */ new Date(d.format(e, this.config.date_format, this.options.language) + " "),
            i
        ];
    }
    bind_grid_click() {
    ...
    bind_bar_events() {
        ...
        if (p.on(this.$svg, "mousemove", ".bar-wrapper, .handle", (g)=>{
            ...
            this.options.move_dependencies ? u = [
                ...
                a
            ], o = u.map((y)=>this.get_bar(y)), this.bar_being_dragged = !1, l = e, o.forEach((y)=>{
                const m = y.$bar;
            ...
            p.on(this.$container, "mousewheel", (_)=>{
                ...
                if (!g && _.currentTarget.scrollLeft <= c) {
                    ...
                    g = !0, this.gantt_start = d.add(this.gantt_start, -this.config.extend_by_units, this.config.unit), this.setup_date_values(), this.render(), _.currentTarget.scrollLeft = u + this.config.column_width * this.config.extend_by_units, setTimeout(()=>g = !1, 300);
                }
                if (!g && _.currentTarget.scrollWidth - (_.currentTarget.scrollLeft + _.currentTarget.clientWidth) <= c) {
                    let u = _.currentTarget.scrollLeft;
                    g = !0, this.gantt_end = d.add(this.gantt_end, this.config.extend_by_units, this.config.unit), this.setup_date_values(), this.render(), _.currentTarget.scrollLeft = u, setTimeout(()=>g = !1, 300);
                }
            });
        }
        p.on(this.$container, "scroll", (g)=>{
            ...
            let u;
            i && (u = g.currentTarget.scrollLeft - i), this.current_date = d.add(this.gantt_start, g.currentTarget.scrollLeft / this.config.column_width * this.config.step, this.config.unit);
            ...
            this.current_date = d.add(this.gantt_start, (g.currentTarget.scrollLeft + m.clientWidth) / this.config.column_width * this.config.step, this.config.unit), y = this.config.view_mode.upper_text(this.current_date, null, this.options.language), m = this.upperTexts.find((w)=>w.textContent === y), m !== this.$current && (this.$current && this.$current.classList.remove("current-upper"), m.classList.add("current-upper"), this.$current = m), i = g.currentTarget.scrollLeft;
            ...
            o.forEach((c)=>{
                ...
                u.finaldx = this.get_snap_position(_, u.ox), this.hide_popup(), s ? a === c.task.id ? c.update_bar_position({
                    ...
                    width: u.owidth + u.finaldx
                }) : t && !this.options.readonly && !this.options.readonly_dates && c.update_bar_position({
                    x: u.ox + u.finaldx
                });
            });
    ...
    get_snap_position(t, e) {
        ...
        for(; l.length;)o += this.config.column_width * h, l = this.get_ignored_region(o, h), l.length || (o -= this.config.column_width * h);
        return o - e;
    }
...
//# sourceMappingURL=web_c796e60d._.js.map