                ...
                : ListView.separated(
                    ...
                    itemBuilder: (c, i) {
                      ...
                      final performed = (r is Map)
                          ...
                          : null;
                      final tech = (r is Map)
                          ? (r['teknisi_name'] ?? r['teknisi_id'] ?? '')
                          : '';
                      return ListTile(
                        title: Text(title.toString()),
                        subtitle: Text(_formatDate(performed) +
                            (tech.toString().isNotEmpty ? ' • $tech' : '')),
                        onTap: () async {
                          // open detail
                          Navigator.push(
                              context,
...
class ChecklistDetailScreen extends StatefulWidget {
  ...
  const ChecklistDetailScreen(
      {required this.checklistId, required this.token, super.key});
...
class _ChecklistDetailScreenState extends State<ChecklistDetailScreen> {
  ...
  Future<void> _fetch() async {
    ...
    try {
      ...
      final res = await api
          .get('/mobile/checklists/${Uri.encodeComponent(widget.checklistId)}');
      if (res is Map) {
        setState(() {
          checklist =
              (res['checklist'] ?? res['data'] ?? res) as Map<String, dynamic>?;
          items = (res['items'] ?? []);
        });
      } else {
        setState(() {
      ...
      body: Padding(
        ...
        child: loading
            ...
            : Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                if (checklist != null) ...[
                  Text(
                      'Alat: ${checklist!['alat']?['nama'] ?? checklist!['alat']?['kode'] ?? '-'}',
                      style: const TextStyle(fontWeight: FontWeight.bold)),
                  const SizedBox(height: 6),
                  Text(
                      'Performed: ${checklist!['performed_at'] ?? checklist!['created_at'] ?? '-'}'),
                  ...
                  Expanded(
                      child: ListView.separated(
                          ...
                          itemBuilder: (c, i) {
                            final it = items[i];
                            final q =
                                it['question'] ?? it['question_id'] ?? 'Q';
                            final ans = it['answer_text'] ??
                                it['answer_number'] ??
                                it['option'] ??
                                '';
                            return ListTile(
