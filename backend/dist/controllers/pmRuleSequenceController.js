...
exports.saveSequenceByAlat = exports.getSequenceByAlat = exports.saveSequenceByJenis = exports.getSequenceByJenis = void 0;
const ormconfig_1 = require("../ormconfig");
async function getSequenceByJenis(req, res) {
    try {
        const jenisId = Number(req.params.jenis_id);
        if (!jenisId)
            return res.status(400).json({ message: 'jenis_id required' });
        const rows = await ormconfig_1.AppDataSource.manager.query(`SELECT seq_no, pm_rule_id FROM pm_rule_sequence WHERE jenis_alat_id = $1 ORDER BY seq_no ASC`, [jenisId]);
...
exports.getSequenceByJenis = getSequenceByJenis;
async function saveSequenceByJenis(req, res) {
    try {
        const jenisId = Number(req.params.jenis_id);
        if (!jenisId)
            return res.status(400).json({ message: 'jenis_id required' });
        const items = req.body.items || [];
        await ormconfig_1.AppDataSource.manager.transaction(async (tm) => {
...
exports.saveSequenceByJenis = saveSequenceByJenis;
async function getSequenceByAlat(req, res) {
    try {
        const alatId = Number(req.params.alat_id);
        if (!alatId)
            return res.status(400).json({ message: 'alat_id required' });
        const rows = await ormconfig_1.AppDataSource.manager.query(`SELECT seq_no, pm_rule_id FROM pm_rule_sequence_alat WHERE alat_id = $1 ORDER BY seq_no ASC`, [alatId]);
        return res.json({ data: rows });
    }
    catch (err) {
...
exports.getSequenceByAlat = getSequenceByAlat;
async function saveSequenceByAlat(req, res) {
    try {
        const alatId = Number(req.params.alat_id);
        if (!alatId)
            return res.status(400).json({ message: 'alat_id required' });
...
exports.saveSequenceByAlat = saveSequenceByAlat;
exports.default = { getSequenceByJenis, saveSequenceByJenis, getSequenceByAlat, saveSequenceByAlat };
