...
export async function listPmRules(req: Request, res: Response) {
  try {
    ...
    const sql = `SELECT * FROM pm_rules ${whereSql} ORDER BY sequence ASC, id ASC LIMIT 1000`;
...
export async function createPmRule(req: Request, res: Response) {
  try {
    ...
    const r = await AppDataSource.manager.query(
      `INSERT INTO pm_rules (kode_rule, description, jenis_alat_id, alat_id, interval_hours, multiplier, start_engine_hour, sequence, active, created_at, updated_at)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9, now(), now()) RETURNING *`,
      [body.kode_rule || null, body.description || null, body.jenis_alat_id || null, body.alat_id || null, body.interval_hours || 0, body.multiplier || 1, body.start_engine_hour || 0, body.sequence != null ? Number(body.sequence) : 0, body.active === false ? false : true]
    );
...
export async function updatePmRule(req: Request, res: Response) {
  try {
    ...
    const body = req.body || {};
    const r = await AppDataSource.manager.query(
      `UPDATE pm_rules SET kode_rule=$1, description=$2, jenis_alat_id=$3, alat_id=$4, interval_hours=$5, multiplier=$6, start_engine_hour=$7, sequence=$8, active=$9, updated_at=now() WHERE id=$10 RETURNING *`,
      [body.kode_rule || null, body.description || null, body.jenis_alat_id || null, body.alat_id || null, body.interval_hours || 0, body.multiplier || 1, body.start_engine_hour || 0, body.sequence != null ? Number(body.sequence) : 0, body.active === false ? false : true, id]
    );
