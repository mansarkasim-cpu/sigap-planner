 ...
 */
async function requestNewToken(): Promise<{ token: string; expiresInSec?: number | null }> {
  if (!AUTH_URL) throw new Error('SIGAP_AUTH_TOKEN_URL not configured');

  // 1) client_credentials (OAuth2)
  if (AUTH_METHOD === 'client_credentials') {
    const body = new URLSearchParams();
    body.append('grant_type', 'client_credentials');
    if (process.env.SIGAP_TOKEN_SCOPE) body.append('scope', process.env.SIGAP_TOKEN_SCOPE);
    ...
    const token = data[TOKEN_FIELD] ?? (data.data && (data.data as any).token) ?? data.access_token;
    const expiresIn = data[EXPIRES_FIELD] ?? (data.data && (data.data as any).expires_in);
  ...
  if (AUTH_METHOD === 'custom') {
    ...
    const resp = await axios.post(AUTH_URL, bodyObj, { headers: headersObj, timeout: 10000 });
    const data: any = resp.data || {};
    ...
    const token =
      (data && (data.data && (data.data as any).token)) ||
      (data && (data as any).token) ||
      (data && (data.data && (data.data as any).access_token)) ||
      (data && (data as any).access_token) ||
      (data && (data as any)[TOKEN_FIELD]);

    // expires could be in various places too
    const expiresIn =
      (data && (data.data && (data.data as any)[EXPIRES_FIELD])) ||
      (data && (data as any)[EXPIRES_FIELD]) ||
      null;
 ...
 */
export async function getSigapToken(): Promise<string> {
  // If static token provided and auto-refresh disabled -> return static
  const staticToken = process.env.SIGAP_BEARER_TOKEN;
  if (staticToken && !AUTO_REFRESH) return staticToken;

  if (isTokenValid()) return cache.token as string;
